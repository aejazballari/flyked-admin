...
export async function limitImageSize({
  imageUrl, maxWidth, maxHeight, mimeType, quality,
}) {
  const image = await createImageDomFromUrl(imageUrl);
  if (image.width <= maxWidth && image.height <= maxHeight) {
    return imageUrl;
  }
  const { width, height } = limitSize(image.width, image.height, maxWidth, maxHeight);
...
export async function getCroppedFile(imageUrl, pixelCrop, maxWidth, maxHeight, mimeType, quality) {
  const { width, height } = limitSize(pixelCrop.width, pixelCrop.height, maxWidth, maxHeight);
  const image = await createImageDomFromUrl(imageUrl);
  const canvas = document.createElement('canvas');
  canvas.width = width;
  canvas.height = height;
